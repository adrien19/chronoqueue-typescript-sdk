...
async function processLongRunningTask(message: any, taskDurationSeconds: number) {
    ...
    try {
        while (running) {
            ...
            try {
                ...
                const heartbeatContext = (client.messages as any).heartbeats?.get(message.messageId);
                if (!heartbeatContext) {
                    leaseExpired = true;
                    console.log('⚠️  Lease expired during processing - skipping acknowledgment');
                } else {
                ...
                if (!leaseExpired) {
                    try {
                        ...
                        if (ackErr.code === 9) { // FAILED_PRECONDITION
                            console.warn('⚠️  Could not acknowledge - message already in terminal state');
                        } else {
                            throw ackErr;
                        }
                    }
                } else {
                    console.log('⚠️  Skipping error acknowledgment - lease already expired');
                }
            } finally {
