...
async function main() {
    ...
    try {
        while (running) {
            const { message, workerId, attemptId, stopHeartbeat } = await client.messages.getNextMessage(
                'email-reminders',
            ...
            console.log(`Processing message ${message.messageId} (worker: ${workerId}, attempt: ${attemptId})`);
