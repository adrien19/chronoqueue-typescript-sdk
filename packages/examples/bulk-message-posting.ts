...
async function main() {
    ...
    try {
        ...
        console.log(`Posting ${messages1.length} messages with ALL_OR_NOTHING mode...`);
        ...
        console.log(`Posting ${messages2.length} messages with BEST_EFFORT mode...`);
        ...
        if (response2.results) {
            ...
            response2.results.forEach((result: MessagePostResult) => {
                if (result.success) {
                    ...
                    console.log(`  ✗ ${result.messageId}: ${errorName} - ${result.error}`);
                }
            });
            console.log();
        ...
        console.log(`Throughput: ${Math.round(messages3.length / (elapsed / 1000))} messages/second`);
        console.log(`Successful: ${response3.successfulCount}/${messages3.length}\n`);
        ...
        const failedMessages = response4.results
            ?.filter(r => !r.success && r.errorCode === BulkMessageErrorCode.INTERNAL_ERROR)
            .map(r => r.messageId) || [];
        ...
        const permanentFailures = response4.results
            ?.filter(r => !r.success && r.errorCode !== BulkMessageErrorCode.INTERNAL_ERROR)
            .map(r => ({ id: r.messageId, error: getErrorCodeName(r.errorCode) })) || [];
