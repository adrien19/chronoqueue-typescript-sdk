...
import { ChronoQueueClient, LeasePolicy, Message, MessageRetentionPolicy_Mode, Queue } from '@chronoqueue/client';
...
async function main() {
    ...
    console.log(`   - Max Extension: ${longRunningLeasePolicy.maxExtension?.seconds}s`);
    console.log(`   - Total Possible: ${Number(longRunningLeasePolicy.baseLease?.seconds) + Number(longRunningLeasePolicy.maxExtension?.seconds)}s`);
    console.log(`   - Heartbeat Timeout: ${longRunningLeasePolicy.heartbeatTimeout?.seconds}s`);
    console.log(`   - Extend Step: ${longRunningLeasePolicy.extendStep?.seconds}s\n`);
    ...
    try {
        ...
        if (err?.message && (/already exists/i.test(err.message) || /duplicate key value violates unique constraint/.test(err.message))) {
    ...
    console.log('📊 Summary:');
    console.log('   - Task 1: 90s task with default queue LeasePolicy (240s max)');
