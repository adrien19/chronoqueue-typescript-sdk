...
const SERVER_ADDRESS = process.env.CHRONOQUEUE_SERVER || 'host.docker.internal:9000';
...
class TripPlannerWorker {
    ...
    async start() {
        ...
        while (this.running) {
            try {
                ...
                const { message, workerId, attemptId, stopHeartbeat } = await this.client.messages.getNextMessage(
                    QUEUE_NAME,
                ...
                await this.processMessage(message, workerId!, attemptId!, stopHeartbeat);
            } catch (error) {
    ...
    private async processMessage(message: Message.Message, workerId: string, attemptId: string, stopHeartbeat?: () => void) {
        const messageId = message.messageId;
        const payload = message.metadata?.payload?.data as PlanTripPayload;
    ...
    private generateAccommodations(request: TripRequest, days: number): Accommodation[] {
    ...
    private updateAverageTime(newTime: number) {
        const total = this.stats.averageProcessingTime * (this.stats.successful - 1) + newTime;
        this.stats.averageProcessingTime = Math.round(total / this.stats.successful);
    }
    ...
    private logStats() {
        ...
        console.log(`   Avg Processing Time: ${(this.stats.averageProcessingTime / 1000).toFixed(2)}s`);
