...
async function main() {
    ...
    const client = new ChronoQueueClient({ connection: { address: SERVER_ADDRESS } });
    ...
    try {
        ...
        const tripRequests: Array<{ request: TripRequest; priority: number; id: string }> = [
            {
                id: 'trip-paris-001',
                priority: 10,
                request: {
        ...
        for (const trip of tripRequests) {
            ...
            console.log(`   Duration: ${trip.request.startDate} → ${trip.request.endDate}\n`);
        }
