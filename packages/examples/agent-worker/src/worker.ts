...
class AgentWorker {
    ...
    async start(): Promise<void> {
        ...
        console.log(`   Heartbeat: ${this.config.enableHeartbeat ? 'enabled' : 'disabled'}`);
        console.log('');
    ...
    async stop(): Promise<void> {
        ...
        console.log(`   Avg Processing Time: ${Math.round(this.stats.avgProcessingTimeMs)}ms`);
        console.log('\n👋 Worker stopped');
    }
    ...
    private async processLoop(): Promise<void> {
        ...
        while (this.running && !this.shutdownRequested) {
            try {
                ...
                // Process the message
                this.processMessage(result.message, result.workerId, result.attemptId);
            } catch (err: any) {
