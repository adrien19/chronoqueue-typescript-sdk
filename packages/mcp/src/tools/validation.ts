 ...
 */
const leasePolicySchema = z.object({
...
export const createQueueSchema = z.object({
    ...
    (data) => data.queue_name || data.name,
...
export const postMessagesBulkSchema = z.object({
    queue_name: z.string().min(1, 'Queue name is required'),
    messages: z.array(bulkMessageSchema).min(1, 'At least one message is required').max(1000, 'Maximum 1000 messages per bulk operation'),
    transaction_mode: z.enum([0, 1] as unknown as [string, ...string[]]).transform(Number).optional(),
});
...
export const createScheduleSchema = z.object({
    ...
    times_of_day: z.array(z.string().regex(/^\d{2}:\d{2}$/, 'Time must be in HH:MM format')).optional(),
...
export const requeueFromDLQSchema = z.preprocess(
    (input: unknown) => {
        if (typeof input === 'object' && input !== null) {
            ...
            // Normalize dlq_name
            if (!obj.dlq_name && obj.queue_name) {
                obj.dlq_name = obj.queue_name;
            }
            // Normalize message_ids
            if (!obj.message_ids && obj.message_id) {
                obj.message_ids = [obj.message_id as string];
            } else if (typeof obj.message_ids === 'string') {
                obj.message_ids = [obj.message_ids];
            }
            return obj;
        }
        return input;
