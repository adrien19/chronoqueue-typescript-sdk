...
const createMockClient = () => {
    ...
    return {
        postMessagesBulk: jest.fn((req, cb) => {
            ...
            else cb(null, { success: true, successfulCount: req.messages.length, failedCount: 0, results: [] });
        }),
            ...
            mockClient.setHandler('postMessagesBulk', (_req: any, cb: any) => {
                cb(null, {
                    ...
                    results: messages.map(msg => ({
                        ...
                        success: true,
                        error: "",
                        errorCode: QueueServiceTypes.PostMessagesBulkResponse_MessagePostResult_ErrorCode.SUCCESS,
                    })),
                });
            });

            const result = await messageClient.postMessagesBulk("test-queue", messages);
    ...
    describe("ALL_OR_NOTHING mode", () => {
        it("should post messages successfully with ALL_OR_NOTHING mode (default)", async () => {
            ...
            expect(mockClient.postMessagesBulk).toHaveBeenCalledWith(
                expect.objectContaining({
                    ...
                    messages: messages,
                    transactionMode: QueueServiceTypes.PostMessagesBulkRequest_TransactionMode.ALL_OR_NOTHING,
                }),
                expect.any(Function)
            );
            expect(result.success).toBe(true);
        ...
        it("should fail all messages when one fails in ALL_OR_NOTHING mode", async () => {
            ...
            mockClient.setHandler('postMessagesBulk', (_req: any, cb: any) => {
                cb(null, {
                    ...
                    results: messages.map(msg => ({
                        ...
                        errorCode: QueueServiceTypes.PostMessagesBulkResponse_MessagePostResult_ErrorCode.VALIDATION_FAILED,
                    })),
                });
            });
            ...
            const result = await messageClient.postMessagesBulk(
                "test-queue",
                messages,
                QueueServiceTypes.PostMessagesBulkRequest_TransactionMode.ALL_OR_NOTHING
            );
    ...
    describe("BEST_EFFORT mode", () => {
        it("should post messages successfully with BEST_EFFORT mode", async () => {
            ...
            expect(mockClient.postMessagesBulk).toHaveBeenCalledWith(
                expect.objectContaining({
                    queueName: "test-queue",
                    messages: messages,
                    transactionMode: QueueServiceTypes.PostMessagesBulkRequest_TransactionMode.BEST_EFFORT,
                }),
                expect.any(Function)
            );
            expect(result.success).toBe(true);
        ...
        it("should handle partial success in BEST_EFFORT mode", async () => {
                ...
                cb(null, {
                    ...
                    results: [
                        ...
                        },
                        {
                            ...
                            errorCode: QueueServiceTypes.PostMessagesBulkResponse_MessagePostResult_ErrorCode.DUPLICATE_MESSAGE_ID,
                        },
                        {
                            messageId: "msg-3",
                            success: true,
                        ...
                        },
                        {
                            ...
                            errorCode: QueueServiceTypes.PostMessagesBulkResponse_MessagePostResult_ErrorCode.SCHEMA_MISMATCH,
                        },
                        {
                            messageId: "msg-5",
                            success: true,
            ...
            expect(result.results[3].errorCode).toBe(
                QueueServiceTypes.PostMessagesBulkResponse_MessagePostResult_ErrorCode.SCHEMA_MISMATCH
            );
        });
            ...
            mockClient.setHandler('postMessagesBulk', (_req: any, cb: any) => {
                cb(null, {
                    ...
                    results: messages.map(msg => ({
                        ...
                        errorCode: QueueServiceTypes.PostMessagesBulkResponse_MessagePostResult_ErrorCode.INTERNAL_ERROR,
                    })),
                });
            });
    ...
    describe("error codes", () => {
        it("should handle VALIDATION_FAILED error code", async () => {
            ...
            expect(result.results[0].errorCode).toBe(
                QueueServiceTypes.PostMessagesBulkResponse_MessagePostResult_ErrorCode.VALIDATION_FAILED
            );
        });
        ...
        it("should handle DUPLICATE_MESSAGE_ID error code", async () => {
            ...
            const result = await messageClient.postMessagesBulk("test-queue", messages);
            expect(result.results[0].errorCode).toBe(
                QueueServiceTypes.PostMessagesBulkResponse_MessagePostResult_ErrorCode.DUPLICATE_MESSAGE_ID
            );
        });
        ...
        it("should handle INTERNAL_ERROR error code", async () => {
            ...
            const result = await messageClient.postMessagesBulk("test-queue", messages);
            expect(result.results[0].errorCode).toBe(
                QueueServiceTypes.PostMessagesBulkResponse_MessagePostResult_ErrorCode.INTERNAL_ERROR
            );
        });
        ...
        it("should handle QUEUE_NOT_FOUND error code", async () => {
            ...
            mockClient.setHandler('postMessagesBulk', (_req: any, cb: any) => {
                cb(null, {
                    ...
                    results: [{
                        ...
                        errorCode: QueueServiceTypes.PostMessagesBulkResponse_MessagePostResult_ErrorCode.QUEUE_NOT_FOUND,
                    }],
                });
            });
            ...
            const result = await messageClient.postMessagesBulk("test-queue", messages);
            expect(result.results[0].errorCode).toBe(
                QueueServiceTypes.PostMessagesBulkResponse_MessagePostResult_ErrorCode.QUEUE_NOT_FOUND
            );
        });
    });
