...
const createMockClient = () => {
    ...
    return {
        postMessage: jest.fn((req, cb) => { if (handlers.postMessage) handlers.postMessage(req, cb); else cb(null, { success: true }); }),
        getNextMessage: jest.fn((req, cb) => { if (handlers.getNextMessage) handlers.getNextMessage(req, cb); else cb(null, { message: null }); }),
        acknowledgeMessage: jest.fn((req, cb) => { if (handlers.acknowledgeMessage) handlers.acknowledgeMessage(req, cb); else cb(null, { success: true }); }),
        cancelMessage: jest.fn((req, cb) => { if (handlers.cancelMessage) handlers.cancelMessage(req, cb); else cb(null, { success: true }); }),
        sendMessageHeartBeat: jest.fn((req, cb) => { if (handlers.sendMessageHeartBeat) handlers.sendMessageHeartBeat(req, cb); else cb(null, { state: 1 }); }),
        renewMessageLease: jest.fn((req, cb) => { if (handlers.renewMessageLease) handlers.renewMessageLease(req, cb); else cb(null, { state: 1 }); }),
        peekQueueMessages: jest.fn((req, cb) => { if (handlers.peekQueueMessages) handlers.peekQueueMessages(req, cb); else cb(null, { messages: [] }); }),
        setHandler: (method: string, handler: Function) => { handlers[method] = handler; },
        clearHandlers: () => { Object.keys(handlers).forEach(k => delete handlers[k]); },
    };
};
...
describe("MessageClient Heartbeat Failure Handling", () => {
    ...
    beforeEach(() => {
        ...
        jest.spyOn(connection, 'getQueueServiceClient').mockReturnValue(mockClient as any);
    ...
    describe("onHeartbeatFailure callback", () => {
        ...
        it("should invoke callback multiple times with incrementing failure count", async () => {
            ...
            expect(failureCallback).toHaveBeenNthCalledWith(1, "test-msg-11", expect.any(Object), 1);
            expect(failureCallback).toHaveBeenNthCalledWith(2, "test-msg-11", expect.any(Object), 2);
            expect(failureCallback).toHaveBeenNthCalledWith(3, "test-msg-11", expect.any(Object), 3);
        });
    ...
    describe("stopAllHeartbeats", () => {
        it("should stop all active heartbeats", async () => {
            ...
            mockClient.setHandler('getNextMessage', (_req: any, cb: any) => {
                messageCounter++;
                cb(null, {
                    message: { messageId: `test-msg-multi-${messageCounter}`, payload: {} },
                    workerId: "worker-1",
            ...
            await messageClient.getNextMessage("queue1", undefined, undefined, true, 1000);
            await messageClient.getNextMessage("queue2", undefined, undefined, true, 1000);
            await messageClient.getNextMessage("queue3", undefined, undefined, true, 1000);
        ...
        it("should log debug messages when stopping heartbeats", async () => {
            ...
            expect(debugLogs.some(args =>
                args[0]?.includes?.("Stopping") && args[0]?.includes?.("active heartbeats")
            )).toBe(true);
            expect(debugLogs.some(args =>
                args[0]?.includes?.("All heartbeats stopped")
            )).toBe(true);
        });
    ...
    describe("Lease expiration handling", () => {
        it("should stop heartbeat when state changes to ERRORED", async () => {
            ...
            expect(messageClient.hasActiveHeartbeat("test-msg-lease-expired")).toBe(true);

            jest.advanceTimersByTime(1000);

            expect(messageClient.hasActiveHeartbeat("test-msg-lease-expired")).toBe(false);
    ...
    describe("Heartbeat success logging", () => {
        it("should log debug message on successful heartbeat with remaining time", async () => {
            ...
            const debugLogs = mockLogger.calls.debug;
            expect(debugLogs.some(args =>
                args[0]?.includes?.("Heartbeat OK") && args[0]?.includes?.("25s remaining")
            )).toBe(true);
        });
    });
