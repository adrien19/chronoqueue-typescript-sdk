...
interface HeartbeatContext {
    ...
    lastError?: Error;
    failedAt?: Date;
    onHeartbeatFailure?: (messageId: string, error: Error, consecutiveFailures: number) => void;
}
...
export class MessageClient {
    ...
    async postMessagesBulk(
        ...
        transactionMode: QueueServiceTypes.PostMessagesBulkRequest_TransactionMode =
        ...
        return this.connection.withRetry(async () => {
            ...
            return new Promise<QueueServiceTypes.PostMessagesBulkResponse>((resolve, reject) => {
            ...
            client.getNextMessage(request, (error, response) => {
                if (error) {
                    ...
                    if (enableHeartbeat && message && message.messageId) {
                        ...
                        const jitteredInterval = this.calculateJitteredInterval(heartbeatIntervalMs);
                        heartbeatTimer = setInterval(() => {
                            ...
                            client.sendMessageHeartBeat(
                                ...
                                (err, heartbeatResponse) => {
                                    if (err) {
                                        ...
                                        if (context.onHeartbeatFailure) {
                                            try {
                                                ...
                                                this.logger.warn('Heartbeat failure callback threw error:', callbackErr);
                                            }
                                        }

                                        // Stop heartbeat on fatal errors or max failures
                                        if (isFatalError || context.consecutiveFailures >= MAX_HEARTBEAT_FAILURES) {
                                            this.logger.warn(
                                                `Stopping heartbeat for messageId: ${message.messageId} - ` +
                                                (isFatalError ? 'fatal error' : `${MAX_HEARTBEAT_FAILURES} consecutive failures`)
                                            );
                                            stopHeartbeat();
                                        }
                                    ...
                                    if (heartbeatResponse?.state === ProtoMessage.Message_Metadata_State.ERRORED ||
                                        heartbeatResponse?.state === ProtoMessage.Message_Metadata_State.PENDING) {
                                        this.logger.warn(`Lease expired for message ${message.messageId} - state changed to ${heartbeatResponse.state}`);
                                    ...
                                    if (heartbeatResponse?.remainingTime) {
                                        this.logger.debug(`Heartbeat OK: ${heartbeatResponse.remainingTime.seconds}s remaining (state: ${heartbeatResponse.state})`);
                                    }
                                }
                            );
                        }, jitteredInterval);
    ...
    async cancelMessage(
        ...
        const heartbeatContext = this.heartbeats.get(messageId);
        if (heartbeatContext) {
            this.logger.debug('Stopping heartbeat for cancelled messageId:', messageId);
            heartbeatContext.stopHeartbeat();
            this.heartbeats.delete(messageId);
        }
    ...
    stopAllHeartbeats(): void {
        ...
        for (const [messageId, context] of heartbeatEntries) {
            try {
                ...
                this.logger.warn(`Error stopping heartbeat for messageId ${messageId}:`, err);
            }
        }
